...
locals {
  ...
  internal_subnets = distinct(
    compact(
      concat(var.accepted_internal_networks, local.default_subnets),
    ),
  )
}
...
module "private_agents" {
  ...
  dcos_instance_os = coalesce(
    var.private_agent_dcos_instance_os,
    var.infra_dcos_instance_os,
  )
...
module "public_agents" {
  ...
  dcos_instance_os = coalesce(
    var.public_agent_dcos_instance_os,
    var.infra_dcos_instance_os,
  )
